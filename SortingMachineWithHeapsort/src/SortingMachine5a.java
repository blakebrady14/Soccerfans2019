...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        /*
         * Declaring left and right subtree indicies
         */
        int left = 2 * top + 1;
        int right = left + 1;

        /*
         * Initially checking if the length of the array is > 1
         */
        if (array.length > 1) {

            /*
             * Because we already checked the length is > 1 and we know it is a
             * heap, do not need to check for left subtree being within range,
             * as it is given
             */
            if (right <= last) {

                /*
                 * Returns -1 if right is bigger, 0 if equal, 1 if left is
                 * bigger
                 */
                int leftGreater = order.compare(array[left], array[right]);

                /*
                 * If right is less than left, right is swapped in
                 */
                if (leftGreater > 0) {
                    /*
                     * Right being swapped if top is > right
                     */
                    ...
                    /*
                     * If there is a right subtree but the left is less than the
                     * right, the following loop replaces the top index with the
                     * left index, then sifts down
                     */
                } else if (left <= last) {
            ...
            /*
             * If left is less than right, left is swapped in, and the previous

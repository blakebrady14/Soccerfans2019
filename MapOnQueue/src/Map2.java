...
public class Map2<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        Map.Pair<K, V> temp = new SimplePair<K, V>(key, value);
    ...
    public final Pair<K, V> remove(K key) {
        ...
        /*
         * Declaring new placeholder pair named Temp to store return value in
         */

        Pair<K, V> temp = new SimplePair<>((K) "", (V) "");

        /*
         * Creating new Instance of this.pairsQueue to add the Pairs into
         */
        Queue<Pair<K, V>> tempQueue = this.pairsQueue.newInstance();

        /*
         * For each loop that iterates through all pairs in this.pairsQueue. If
         * the pair does not contain the key "key" it is not added and that pair
         * is assigned to the temporary pair to return, but else, it is added
         */
        for (Pair<K, V> tempP : this.pairsQueue) {
            if (!(tempP.key().equals(key))) {
                tempQueue.enqueue(tempP);
            } else {
                temp = tempP;
            }
        }
        ...
        /*
         * Transferring the copyQueue into the original Queue
         */
        this.pairsQueue.transferFrom(tempQueue);
    ...
    public final V value(K key) {
        ...
        V check = (V) "";
        for (Pair<K, V> temp : this.pairsQueue) {
            if (temp.key().equals(key)) {
                check = temp.value();
            }
        }
